...
test ("page renders correctly", async () => {
    ...
    const {getByText, getByAltText, getByTestId, queryAllByRole} = render(<App />);
    getByText(/fetching data/i);
    //await for the data to be fetched
    await waitFor(() => {
        expect(mockFetchShow).toHaveBeenCalledTimes(1);

        getByAltText(/stranger things/i);

        getByTestId("description");
        
        getByText(/one very strange little girl/i);
    })
    // Click the dropdown
    userEvent.click(getByText(/select a season/i));
    // We should have 4 seasons in the dropdown list
